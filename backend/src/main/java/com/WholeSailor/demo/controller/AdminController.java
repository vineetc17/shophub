...
public class AdminController {
   ...
   /* @PutMapping("/shopkeepers/{shop_reqID}/action/{stat}")
    ...
    ResponseEntity<String> DeleteCategory(@RequestHeader("user-id") int user_id, @RequestBody List<Integer> cids) {
        if (adminDAO.isAdmin(user_id)) {
            int res = adminDAO.deleteCategory(cids);
            if (res == 1) {
                return new ResponseEntity<>("Categories have been deleted", HttpStatus.OK);
            } else {
                return new ResponseEntity<>("Something went wrong", HttpStatus.BAD_GATEWAY);
            }
        } else {
